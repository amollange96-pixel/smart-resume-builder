...
export const mockSignInWithPopup = async (authObj: any, provider: any) => {
  const email = prompt("Enter your Google/Email address to sign in locally:", "your-email@gmail.com");
  if (email === null) return { user: null }; // Cancelled
  const normalizedEmail = email.trim() || "developer@example.com";
  
  const defaultName = normalizedEmail.split('@')[0];
  const displayName = prompt("Enter your Display Name:", defaultName) || defaultName;
