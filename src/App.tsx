  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        try {
          await signInWithEmailAndPassword(auth, 'developer@example.com', 'password');
        } catch (error) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 relative">
        ...
        >
          ...
          <div className="w-20 h-20 bg-gray-800/80 rounded-2xl flex items-center justify-center mx-auto mb-6 border border-gray-700 shadow-2xl relative z-10">
            <AlertCircle className="w-10 h-10 text-red-500 animate-pulse" />
          </div>
          
          <h1 className="text-2xl font-black text-white mb-2 relative z-10">Authentication Required</h1>
          <p className="text-gray-400 text-sm font-medium relative z-10 leading-relaxed">
            Failed to automatically authenticate. Please refresh the page or check your connection.
          </p>
        </motion.div>
      </div>
    );
    ...
    <div className="min-h-screen flex relative">
      ...
      <aside className="w-72 glass-panel border-r-0 border-r border-gray-800 flex flex-col fixed h-full z-20">
        ...
        <div className="p-6 border-t border-white/5">
          <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 backdrop-blur-xl transition-all hover:bg-white/10 cursor-pointer">
